...
import {
  approveDayOffSwapRequestBatch,
  getDayOffSwapRequestBatch,
  rejectDayOffSwapRequestBatch,
} from '../../api/dayOffSwapRequests'
import { useRefreshPendingApprovals } from '../../context/pendingApprovalsContext'
import { notify } from '../../notifications/notify'
import { alert, alertDetail, alertTitle, badge, button, card, eyebrow, fieldControl, link, muted, pageHead, subtitle } from '../../styles'

type State =
  | { phase: 'loading' }
  | { phase: 'ok'; requests: DayOffSwapRequestListItem[]; canDecideBatch: boolean }
...
/**
 * One "ขอสลับวันหยุดแบบกลุ่ม" submission, as the group of independent
 * day_off_swap_requests rows it actually is (see migration 083's comment —
 * there is no batch table). "อนุมัติทั้งหมด"/"ปฏิเสธทั้งหมด" act on every row
 * still pending in one call; a row a batch decision could not settle (a
 * stale calendar/shift drift, most often) stays pending here with its reason
 * shown, for a reviewer to open individually — this page has no per-row
 * decide button itself, since going to the request's own detail page can
 * also correct whatever went stale before deciding it. Mirrors
 * OvertimeRequestBatchDetailPage almost exactly.
 */
export function DayOffSwapRequestBatchDetailPage() {
  ...
  useEffect(() => {
    ...
    getDayOffSwapRequestBatch(batchId, controller.signal)
  ...
  async function refetch() {
    if (!batchId) return
    try {
      const { requests, canDecideBatch } = await getDayOffSwapRequestBatch(batchId)
  ...
  async function handleApproveAll() {
    ...
    setBusy(true)
    try {
      const outcomes = await approveDayOffSwapRequestBatch(batchId)
  ...
  async function handleRejectAll(event: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const outcomes = await rejectDayOffSwapRequestBatch(batchId, rejectReason)
      const okCount = outcomes.filter((o) => o.kind === 'ok').length
    ...
    <>
      <header className={pageHead}>
        <div>
          ...
          <h1>รายละเอียดคำขอสลับวันหยุดแบบกลุ่ม</h1>
          <p className={subtitle}>รายชื่อพนักงานทุกคนที่ถูกขอสลับวันหยุดพร้อมกันในคำขอนี้</p>
        </div>
          ...
          <div className="mb-4 flex items-center justify-between gap-4">
            <div>
              <p className="font-semibold text-slate-900">
                {formatDate(first.offDate)} (หยุด) → {formatDate(first.workDate)} (ทำงาน)
              </p>
              <p className={muted}>
                {first.createdByName ? `ขอแทนโดย ${first.createdByName}` : 'ขอโดยพนักงานเอง'} · {state.requests.length}{' '}
                คน
              </p>
            </div>
          </div>
        ...
        <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse text-[0.825rem] [&_tbody_tr:last-child_td]:border-b-0">
              <thead>
                <tr>
                  {['รหัสพนักงาน', 'ชื่อพนักงาน', 'สถานะ', 'ขั้นตอน'].map((h) => (
                    <th
                      key={h}
                  ...
                  <tr key={request.id}>
                    ...
                    <td className="border-b border-slate-200 px-4 py-2.5 align-middle">
                      <div className="flex items-center gap-2">
                        ...
                        <Link className={link} to={`/day-off-swap-requests/${request.id}`}>
                          เปิดคำขอนี้
                        </Link>
                      </div>
                    </td>
