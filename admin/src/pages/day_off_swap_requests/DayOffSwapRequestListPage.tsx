...
type State =
  | { phase: 'loading' }
  | { phase: 'ok'; requests: DayOffSwapRequestListItem[]; total: number }
  | { phase: 'error'; message: string }

/** Matches the server's own default in dayOffSwapRequestQueries.ts. */
 ...
 *  member of one "ขอสลับวันหยุดแบบกลุ่ม" submission collapsed into a single
 *  row (batchId is shared, see migration 083's comment for why there is no
 *  batch table to query this from directly). Grouped client-side rather than
 *  by the list endpoint: the endpoint's shape and sorting stay useful for
 *  every other caller. Mirrors OvertimeRequestListPage's groupByBatch. */
type DisplayRow =
  | { kind: 'single'; request: DayOffSwapRequestListItem }
  | { kind: 'batch'; batchId: string; requests: DayOffSwapRequestListItem[] }

function groupByBatch(requests: DayOffSwapRequestListItem[]): DisplayRow[] {
...
export function DayOffSwapRequestListPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve(hasEmployeeFilter ? resolveEmployeeIds(employeeFilters.filter, controller.signal) : undefined)
      .then((employeeIds) =>
        tab === 'mine'
          ? listDayOffSwapRequestsPendingApproval(controller.signal).then((requests) => {
          ...
          : listDayOffSwapRequests(tab === 'all' ? undefined : tab, { page, pageSize }, controller.signal, employeeIds)
      )
      .then((body) => {
  ...
  const displayRows = useMemo(() => (state.phase === 'ok' ? groupByBatch(state.requests) : []), [state])
    ...
    <>
      <header className={pageHead}>
        ...
        <button
          ...
          type="button"
          onClick={() => void navigate('/day-off-swap-requests/bulk-request')}
        >
          ขอสลับวันหยุดแบบกลุ่ม
        </button>
      </header>
                ...
                <table className="w-full border-collapse text-[0.825rem] [&_tbody_tr:last-child_td]:border-b-0">
                  ...
                  <tbody>
                    {displayRows.map((row, index) => {
                      ...
                      // A batch row: every member shares workDate/offDate/
                      // reason (one bulk submission, one set of details), so
                      // the first member stands in for them. Status can
                      // differ per member once a batch decision leaves some
                      // rows 'stale' — every distinct status present gets its
                      // own badge rather than picking one.
                      ...
                      return (
                        <tr
                          key={row.batchId}
                          onClick={() => void navigate(`/day-off-swap-requests/batch/${row.batchId}`)}
                        ...
                        >
                          ...
                          <td className="border-b border-slate-200 px-4 py-2.5 align-middle whitespace-nowrap text-slate-600 tabular-nums">
                            {formatDate(first.workDate)}
                          </td>
                          <td className="border-b border-slate-200 px-4 py-2.5 align-middle whitespace-nowrap text-slate-600 tabular-nums">
                            {formatDate(first.offDate)}
                          </td>
                          <td className="max-w-64 truncate border-b border-slate-200 px-4 py-2.5 align-middle text-slate-600">
                            {first.reason}
                          </td>
