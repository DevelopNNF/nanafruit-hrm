...
import type { CompTimeOffRequestListItem } from '@hrm/shared'
import { approveCompTimeOffRequest, getCompTimeOffRequest, rejectCompTimeOffRequest } from '../../api/compTimeOffRequests'
...
type State =
  | { phase: 'loading' }
  | { phase: 'ok'; request: CompTimeOffRequestListItem; canDecide: boolean }
...
function statusBadgeTone(
  status: CompTimeOffRequestListItem['status']
...
function formatOffDate(request: CompTimeOffRequestListItem): string {
  return `${formatDate(request.offDate)} เวลา ${request.startTime.slice(0, 5)}–${request.endTime.slice(0, 5)}`
}
...
export function CompTimeOffRequestDetailPage() {
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    getCompTimeOffRequest(requestId, controller.signal)
  ...
  async function handleApprove() {
    ...
    setBusy(true)
    try {
      const { request, canDecide } = await approveCompTimeOffRequest(state.request.id)
      setState({ phase: 'ok', request, canDecide })
      notify.success(
        request.status === 'pending' ? 'ส่งต่อให้ HR/Admin แล้ว' : 'อนุมัติคำขอแล้ว',
        request.status === 'pending' ? undefined : 'หักยอดวันหยุดสะสมของพนักงานแล้ว'
      )
    } catch (err) {
      ...
      getCompTimeOffRequest(state.request.id)
        .then(({ request, canDecide }) => setState({ phase: 'ok', request, canDecide }))
        .catch(() => {})
    } finally {
      setBusy(false)
  ...
  async function handleReject(event: React.FormEvent) {
    ...
    setBusy(true)
    try {
      const { request, canDecide } = await rejectCompTimeOffRequest(state.request.id, rejectReason)
      setState({ phase: 'ok', request, canDecide })
      setRejecting(false)
    ...
    <>
      <header className={pageHead}>
        <div>
          ...
          <h1>รายละเอียดคำขอแปลง OT เป็นวันหยุด</h1>
          <p className={subtitle}>ตรวจสอบและอนุมัติ/ปฏิเสธคำขอใช้วันหยุดสะสมจากพนักงาน</p>
        </div>
        <Link className={link} to="/comp-time-off-requests">
          ← กลับไปรายการคำขอ
        </Link>
      </header>
      ...
      {state.phase === 'ok' && (
        <div className={`${card} max-w-2xl`}>
          ...
          <dl className={spec}>
            <dt className={specDt}>วันที่ขอหยุด</dt>
            <dd className={specDd}>{formatOffDate(state.request)}</dd>

            <dt className={specDt}>จำนวนชั่วโมง</dt>
            <dd className={specDd}>{formatHours(state.request.requestedMinutes)} ชั่วโมง</dd>
