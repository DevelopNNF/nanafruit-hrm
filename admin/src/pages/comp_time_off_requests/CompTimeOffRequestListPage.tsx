...
import type { CompTimeOffRequestListItem, CompTimeOffRequestStage, CompTimeOffRequestStatus } from '@hrm/shared'
import { listCompTimeOffRequests, listCompTimeOffRequestsPendingApproval } from '../../api/compTimeOffRequests'
...
type State =
  | { phase: 'loading' }
  | { phase: 'ok'; requests: CompTimeOffRequestListItem[]; total: number }
  | { phase: 'error'; message: string }

/** Matches the server's own default in compTimeOffRequestQueries.ts. */
const DEFAULT_PAGE_SIZE = 50

// 'mine' is not a CompTimeOffRequestStatus — it's the caller's own supervisor
// inbox, fetched from a different endpoint entirely (see the effect below).
// It stays unpaginated: that inbox is naturally bounded to whatever is
// currently pending on this account, unlike the other tabs which accumulate
// every request ever decided.
type TabValue = CompTimeOffRequestStatus | 'all' | 'mine'
...
const STATUS_LABEL: Record<CompTimeOffRequestStatus, string> = {
  pending: 'รอดำเนินการ',
...
const STAGE_LABEL: Record<CompTimeOffRequestStage, string> = {
  supervisor: 'รอหัวหน้างาน',
  hr: 'รอ HR/Admin',
}

function statusBadgeTone(status: CompTimeOffRequestStatus): 'pending' | 'active' | 'danger' | 'inactive' {
...
function formatOffDate(request: CompTimeOffRequestListItem): string {
  return `${formatDate(request.offDate)} (${request.startTime.slice(0, 5)}–${request.endTime.slice(0, 5)})`
}
...
export function CompTimeOffRequestListPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve(hasEmployeeFilter ? resolveEmployeeIds(employeeFilters.filter, controller.signal) : undefined)
      .then((employeeIds) =>
        tab === 'mine'
          ? listCompTimeOffRequestsPendingApproval(controller.signal).then((requests) => {
          ...
          : listCompTimeOffRequests(tab === 'all' ? undefined : tab, { page, pageSize }, controller.signal, employeeIds)
      )
      .then((body) => {
  ...
  return (
    <>
      <header className={pageHead}>
        <div>
          <p className={eyebrow}>Comp Time Off</p>
          <h1>คำขอแปลง OT เป็นวันหยุด</h1>
          <p className={subtitle}>คำขอใช้วันหยุดสะสมจากพนักงาน รออนุมัติหรือปฏิเสธ</p>
        </div>
      </header>
            ...
            <div className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse text-[0.825rem] [&_tbody_tr:last-child_td]:border-b-0">
                  <thead>
                    <tr>
                      {['#', 'รหัสพนักงาน', 'ชื่อพนักงาน', 'วันที่ขอหยุด', 'จำนวนชั่วโมง', 'เหตุผล', 'สถานะ', 'ขั้นตอน'].map(
                        (h) => (
                          <th
                            key={h}
                  ...
                  <tbody>
                    {state.requests.map((request, index) => (
                      <tr
                        key={request.id}
                        onClick={() => void navigate(`/comp-time-off-requests/${request.id}`)}
