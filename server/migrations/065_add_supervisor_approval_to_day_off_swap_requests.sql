-- Same optional supervisor-approval stage as 062/063/064, here for
-- day_off_swap_requests. See 062's comment for the full reasoning behind
...
UPDATE day_off_swap_requests SET current_stage = 'hr' WHERE status = 'pending';

ALTER TABLE day_off_swap_requests
  ADD CONSTRAINT day_off_swap_requests_stage_consistency CHECK (
    (status = 'pending') = (current_stage IS NOT NULL)
  ),
  ADD CONSTRAINT day_off_swap_requests_supervisor_requirement CHECK (
    requires_supervisor_approval = (supervisor_employee_id IS NOT NULL)
  ),
  ADD CONSTRAINT day_off_swap_requests_supervisor_stage_pending CHECK (
    current_stage IS DISTINCT FROM 'supervisor' OR supervisor_approved_by_oid IS NULL
  ),
  ADD CONSTRAINT day_off_swap_requests_supervisor_approval_consistency CHECK (
  ...
  ADD CONSTRAINT day_off_swap_requests_supervisor_approval_requires_flag CHECK (
    supervisor_approved_by_oid IS NULL OR requires_supervisor_approval
  );

CREATE INDEX day_off_swap_requests_supervisor_pending_idx
  ON day_off_swap_requests (supervisor_employee_id, created_at DESC)
