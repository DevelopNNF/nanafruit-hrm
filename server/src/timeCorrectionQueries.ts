...
/** Default and max rows per page for listTimeCorrections — same numbers as
 *  listAttendanceDaily's. */
const DEFAULT_PAGE_SIZE = 50
const MAX_PAGE_SIZE = 200

export type TimeCorrectionsPagination = {
...
export async function listTimeCorrections(
  filter: { status?: TimeCorrectionStatus; employeeIds?: number[] },
  pagination: TimeCorrectionsPagination = {},
  db: Queryable = pool
): Promise<{ requests: TimeCorrectionListItem[]; page: number; pageSize: number; total: number }> {
  ...
  if (filter.employeeIds && filter.employeeIds.length > 0) {
    ...
    conditions.push(`t.employee_id = ANY($${params.length})`)
  }
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : ''

  const [listResult, countResult] = await Promise.all([
    db.query<TimeCorrectionListRow>(
      `${SELECT_TIME_CORRECTION_LIST} ${where} ORDER BY t.created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, pageSize, offset]
    ),
    db.query<{ total: string }>(`SELECT count(*) AS total FROM time_correction_requests t ${where}`, params),
  ])

  return {
    requests: listResult.rows.map(rowToTimeCorrectionListItem),
    page,
    pageSize,
