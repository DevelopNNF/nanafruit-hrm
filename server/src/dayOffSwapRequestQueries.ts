...
/** Default and max rows per page for listDayOffSwapRequests — same numbers as
 *  listAttendanceDaily's. */
const DEFAULT_PAGE_SIZE = 50
const MAX_PAGE_SIZE = 200

export type DayOffSwapRequestsPagination = {
...
export async function listDayOffSwapRequests(
  filter: { status?: DayOffSwapRequestStatus; employeeIds?: number[] },
  pagination: DayOffSwapRequestsPagination = {},
  db: Queryable = pool
): Promise<{ requests: DayOffSwapRequestListItem[]; page: number; pageSize: number; total: number }> {
  ...
  if (filter.status !== undefined) {
    ...
    conditions.push(`dosr.status = $${params.length}`)
  }
  if (filter.employeeIds && filter.employeeIds.length > 0) {
    params.push(filter.employeeIds)
    conditions.push(`dosr.employee_id = ANY($${params.length})`)
  }
  const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : ''

  const [listResult, countResult] = await Promise.all([
    db.query<DayOffSwapRequestListRow>(
      `${SELECT_DAY_OFF_SWAP_REQUEST_LIST} ${where} ORDER BY dosr.created_at DESC LIMIT $${params.length + 1} OFFSET $${params.length + 2}`,
      [...params, pageSize, offset]
    ),
    db.query<{ total: string }>(`SELECT count(*) AS total FROM day_off_swap_requests dosr ${where}`, params),
  ])

  return {
    requests: listResult.rows.map(rowToDayOffSwapRequestListItem),
    page,
    pageSize,
